...
MAX_LEVELS = 3
MISSIONS = {1: 'Welcome To HCMUT', 2: "      Into The HCMUT", 3: "    Meet The Devora"}
...
# Buttons
start_img = pygame.image.load(f'assets/img/buttons/start.png').convert_alpha()
restart_img = pygame.image.load(f'assets/img/buttons/restart_btn.png').convert_alpha()
exit_img = pygame.image.load(f'assets/img/buttons/exit.png').convert_alpha()
option_img = pygame.image.load(f'assets/img/buttons/option.png').convert_alpha()
intro_bg = pygame.image.load(f"assets/img/buttons/intro.png").convert_alpha()
onSound = pygame.image.load(f"assets/img/buttons/onsound.png").convert_alpha()
offSound = pygame.image.load(f"assets/img/buttons/offsound.png").convert_alpha()
...
class DropItem(pygame.sprite.Sprite):
    def __init__(self, item_type, x, y, scale):
        ...
        self.image = pygame.image.load(f'assets/img/drop_item/{item_type}.png').convert_alpha()
        self.rect = self.image.get_rect()
        self.rect.center = (x, y)
        self.width = self.image.get_width()
        self.height = self.image.get_height()
        ...
        if self.rect.colliderect(player):
            if player.alive:
                if self.item_type == "star":
                    ...
                    player.coin += 1
                    damage_text = DamageText(self.rect.centerx,
                                            self.rect.centery, str(" +1 coin "), WHITE)
                    damage_text_group.add(damage_text)
                self.kill()
...
class LiveEntity(pygame.sprite.Sprite):
    ...
    def update(self):
        ...
        if pygame.sprite.spritecollide(self, bullet_group, False):
            if self.alive and self.entity_type == "chest":
                self.health -= 25
                # drop star item
                star = DropItem('star', self.rect.centerx, self.rect.centery, 2)
                drop_item_group.add(star)
            ...
            if pygame.sprite.spritecollide(enemy, bullet_group, False):
                if enemy.alive:
                    ...
                    if enemy.health > 0:
                        ...
                        # drop coin on death
                        coin = DropItem('coin', enemy.rect.x, enemy.rect.y, 2)
                        print("coin droped \n")
                        drop_item_group.add(coin)
                    self.kill()
...
# create buttons
start_btn = button.Button(SCREEN_WIDTH // 2 - 130, SCREEN_HEIGHT // 2 - 250, start_img, 1)
exit_btn = button.Button(SCREEN_WIDTH // 2 - 130, SCREEN_HEIGHT // 2 - 110, exit_img, 1)
option_btn = button.Button(SCREEN_WIDTH // 2 - 130, SCREEN_HEIGHT //2 - 10, option_img, 1)
restart_btn = button.Button(SCREEN_WIDTH // 2 - 100, SCREEN_HEIGHT // 2 - 50, restart_img, 2)
onSound_btn = button.Button(SCREEN_WIDTH // 2 - 50, SCREEN_HEIGHT // 2 - 250, onSound, 1)
offSound_btn = button.Button(SCREEN_WIDTH // 2 - 50, SCREEN_HEIGHT // 2 - 110, offSound, 1)
...
drop_item_group = pygame.sprite.Group()
...
while running:
    ...
    if start_game == False:
        ...
        # add buttons
        if option_scr:
            if onSound_btn.draw(screen):
                sound = True
            if offSound_btn.draw(screen):
                sound = False
        ...
        drop_item_group.update()
        # draw damage text
        live_entity_group.draw(screen)
        drop_item_group.draw(screen)
